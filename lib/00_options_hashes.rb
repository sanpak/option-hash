...
def transmogrify(word,option = {})
  ...
  defaults = defaults.merge(option)
  result = word * defaults[:times]
  result.upcase! if defaults[:upcase]
  result.reverse! if defaults[:reverse]
  return result
end
